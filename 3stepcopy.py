...
# Function to calculate y_min and y_max for each staircase group
def calculate_min_max_y(group):
    y_values = [coord[1] for coord in group['coordinates']]
    y_min = min(y_values)
    y_max = max(y_values)
    return y_min, y_max
...
# Calculate y_min and y_max for each staircase group
for group in ankle_groups:
    group['y_min'], group['y_max'] = calculate_min_max_y(group)
        ...
        for group in ankle_groups:
            if y_min <= left_ankle_y <= y_max and y_min <= right_ankle_y <= y_max:
                ...
                for coord in group['coordinates']:
                    if (left_ankle_y, right_ankle_y) == coord:
                ...
                # Draw a rectangle for each staircase (based on the y_min, y_max for each group)
                cv2.rectangle(frame, (0, group['y_min']), (width, group['y_max']), (0, 255, 0), 2)
                cv2.putText(frame, f"Staircase {ankle_groups.index(group) + 1}", (10, group['y_min'] + 20), 
